#!/usr/bin/env python3
"""Produce this mod's three 8-bit BMPs into `src/` from the game's own screen art.

The sources are read from this script's folder, under the names they have inside
`p2arch0_eng.cpr` (extract them with `cprcli`), each the largest the game ships for its
piece:

    Vollansichtskarte.bmp    2400 x 1800, the full map master
    HauptscreenA1280.bmp      996 x 42,   the top bar
    HauptscreenE1280.bmp      284 x 424,  the right-hand bottom piece
...
284 x (HEIGHT - 600). The map is stretched to the screen rather than fitted, which is
what the game's own set does - it stores the same map as 4:3 and 5:4 images for its
resolutions. The palette is kept: with Pillow installed the pixels are resampled
in RGB and quantised back to the source palette; without it a nearest-neighbour resample
of the palette indices is used.

Usage: resize_images.py [width height]   (default 1664 936)
"""
import os
import struct
import sys
...
WIDTH, HEIGHT = (int(sys.argv[1]), int(sys.argv[2])) if len(sys.argv) == 3 else (1664, 936)
TAG = str(WIDTH)

JOBS = [
    ("Vollansichtskarte.bmp", f"Vollansichtskarte{TAG}.bmp", WIDTH, HEIGHT),
    ("HauptscreenA1280.bmp", f"HauptscreenA{TAG}.bmp", WIDTH - 284, 42),
    ("HauptscreenE1280.bmp", f"HauptscreenE{TAG}.bmp", 284, HEIGHT - 600),
]
...
missing = [source for source, _, _, _ in JOBS if not os.path.exists(os.path.join(HERE, source))]
if missing:
    sys.exit(f"missing in {HERE}: {', '.join(missing)} - extract them from p2arch0_eng.cpr with cprcli")

for source, target, w, h in JOBS:
    palette, rows = read_bmp8(os.path.join(HERE, source))
