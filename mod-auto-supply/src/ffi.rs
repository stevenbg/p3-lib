...
use hooklet::windows::x86::{deploy_rel32_raw, hook_call_rel32, hook_call_rel32_with_module, hook_function_pointer, hook_function_pointer_width_module, CallRel32Hook, FunctionPointerHook, X86Rel32Type};
...
use windows::Win32::{
    ...
    System::Diagnostics::Debug::{GetThreadContext, CONTEXT},
    System::LibraryLoader::{GetModuleFileNameA, GetModuleHandleExA, GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT},
    System::Memory::{VirtualQuery, MEMORY_BASIC_INFORMATION, MEM_COMMIT},
    System::Threading::{GetCurrentThreadId, OpenThread, ResumeThread, SuspendThread, THREAD_GET_CONTEXT, THREAD_SUSPEND_RESUME},
    UI::{
...
pub unsafe extern "C" fn start() -> u32 {
    ...
    // draw vtable slots.
    if let Err(code) = install_perf_probe() {
        return code;
    }
    // The probe's sampling profiler: needs the main thread's id (start() runs on it).
...
/// F9 (THROWAWAY): a per-second frame profiler, for the vanilla framerate drop the
/// shipyard window causes even on an empty page. Three hooks, installed at start():
///
...
///   code with rdtsc, converted to ms via the bucket's measured cycles-per-ms.
///
/// F9 toggles the per-second log lines; counting always runs (it is cheap). Compare
/// the lines with the window closed vs open; keep extra speed at x1 while measuring,
/// because mod-ui-tweaks scales `[0x6DCCF4]`.
...
/// F9 enable), so a run can be grepped from WSL afterwards instead of pasted.
...
/// decode storm. ALT+F9 forces such colors to opaque white to A/B this.
...
unsafe fn install_perf_probe() -> Result<(), u32> {
...
/// The probe hooks into other modules (ddraw_dll.dll, aim.dll), installed lazily on
/// the first F9 press: at start() time the modloader may run before those DLLs are
...
unsafe fn install_perf_late_hooks() {
    ...
    // long after start() - hooking here (first F9) is safe from being overwritten.
...
unsafe fn on_perf_probe_hotkey() {
    let on = !PERF_PROBE_ON.load(Ordering::SeqCst);
    if on {
    ...
    notify(&format!("perf probe: {}", if on { "on" } else { "off" }));
}

/// One-shot on probe enable: walk the local-map scene's graphic-object table
...
unsafe extern "C" fn on_perf_frame() {
    ...
    if PERF_PROBE_ON.load(Ordering::Relaxed) {
        ...
        perf_log!(
            ...
            p.frames,
            p.delta_max,
            *(0x05080F14 as *const u32) / 1024,
            *(0x0505F734 as *const u32) / 1024,
            p.submits,
        ...
        if !p.decode_dll_sites.is_empty() {
            ...
            let list: Vec<String> = p.decode_dll_sites.iter().take(8).map(|(site, count)| format!("{site:#010x} x{count}")).collect();
...
unsafe extern "cdecl" fn aim_decode_file_perf_hook(image_inner: u32, file_path: *const u8, file_data: u32, file_size: u32) -> i32 {
    ...
    while cursor < ceiling {
        ...
        if trigger == 0 && (0x401000..0x66A000).contains(&value) {
            trigger = value;
        }
        if dll_found < 3 && (0x5001000..0x5050000).contains(&value) && !dll_sites.contains(&value) {
            dll_sites[dll_found] = value;
            dll_found += 1;
        }
...
unsafe extern "C" fn free_memory_texture_perf_hook(texture: u32) -> u32 {
    ...
    while cursor < ceiling {
        let value = *(cursor as *const u32);
        if (0x401000..0x66A000).contains(&value) {
            site = value;
            break;
        }
        cursor += 4;
...
unsafe extern "C" fn on_perf_record_dtor(record: u32) {
    ...
    while cursor < ceiling {
        let value = *(cursor as *const u32);
        if (0x401000..0x66A000).contains(&value) && !(0x4B2000..0x4B5000).contains(&value) {
            site = value;
            break;
        }
        cursor += 4;
...
/// ALT+F9: toggle the sgl decoded-image cache budget between the game's value and
/// 512 MB. The LRU evictor at ddraw_dll 0x501C2EF/0x501C39A frees images while
/// usage [0x5080F14] exceeds the limit [0x505F734] (default 16 MB, gl.cfg
/// "TextureCacheSize" in the D3D section = 48 MB) - too small for the town scene
/// plus an open building window, causing the per-frame re-decode storm.
static SAVED_CACHE_LIMIT: AtomicU32 = AtomicU32::new(0);

unsafe fn on_cache_budget_hotkey() {
    let limit = 0x0505F734 as *mut u32;
    if SAVED_CACHE_LIMIT.load(Ordering::SeqCst) == 0 {
        SAVED_CACHE_LIMIT.store(*limit, Ordering::SeqCst);
        *limit = 0x2000_0000;
        notify(&format!("texture cache budget: 512MB (was {})", SAVED_CACHE_LIMIT.load(Ordering::SeqCst)));
    } else {
        *limit = SAVED_CACHE_LIMIT.load(Ordering::SeqCst);
        SAVED_CACHE_LIMIT.store(0, Ordering::SeqCst);
        notify(&format!("texture cache budget: restored ({})", *limit));
    }
}
